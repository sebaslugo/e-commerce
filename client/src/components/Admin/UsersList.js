...
const UsersList = () => {
    ...
    const handlePromoteClick = (id, rol) => {
        if (rol == 'admin') {
            return alert('Este usuario ya es administrador.');
        } else {
            dispatch(promoteUser(id));
            return alert('Usuario promovido a administrador, exitosamente.');
        }
    }

    const handleDeleteClick = (id) => {
        if (localStorage.getItem('idUser') == id) {
            return alert('No puede eliminar su propio usuario, contactar al Root Admin');
        } else {
            dispatch(deleteUser(id));
            return alert('Usuario eliminado exitosamente');
        }        
    }
